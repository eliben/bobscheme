...
* ``barevm``: BareVM source code. See the `BareVM`_ section
  for more details.
...
To build the ``barevm`` binary, ``cd`` into ``barevm`` and run ``make``.

Running and testing
-------------------

The main BareVM driver (implemented in ``main.cpp``) is very simple - it
...
The most comprehensive testing on BareVM are done by running the full tests.
