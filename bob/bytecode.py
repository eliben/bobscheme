...
from .utils import pack_word, unpack_word, get_bytes_from_iterator
...
class CodeObject(object):
    ...
    def __repr__(self, nesting=0):
        ...
        repr += prefix + "----------\n"
        repr += (
            prefix + "CodeObject: " + ("" if self.name is None else self.name) + "\n"
        )
        ...
        for offset, instr in enumerate(self.code):
            ...
            if instr.opcode == OP_CONST:
                repr += "%4s {= %s}\n" % (
                    instr.arg,
                    expr_repr(self.constants[instr.arg]),
                )
...
class Serializer(object):
    ...
    def _s_symbol(self, symbol):
        return (
            TYPE_SYMBOL + self._s_word(len(symbol.value)) + symbol.value.encode("ascii")
        )

    def _s_pair(self, pair):
        return TYPE_PAIR + self._s_object(pair.first) + self._s_object(pair.second)
...
class Deserializer(object):
    ...
    def _match_type(self, stream, typ):
        b = next(stream)
        if bytes([b]) != typ:
            raise self.DeserializationError("Expected type '%s' not matched" % typ)
