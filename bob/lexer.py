...
class Lexer(object):
    ...
    def __init__(self, rules, skip_whitespace=True):
        """ Create a lexer.
            ...
            rules:
                A list of rules. Each rule is a `regex, type` pair, where 
                `regex` is the regular expression used to recognize the token 
                and `type` is the type of the token to return when it's 
                recognized.
            
            skip_whitespace:
                If True, whitespace (\s+) will be skipped and not reported by 
                the lexer. Otherwise, you have to specify your rules for 
                whitespace, or it will be flagged as an error.
        """
        # All the regexes are concatenated into a single one with named groups. 
        # Since the group names must be valid Python identifiers, but the token 
        # types used by the user are arbitrary strings, we auto-generate the 
        # group names and map them to token types.
        #
        idx = 1
        regex_parts = []
        self.group_type = {}
